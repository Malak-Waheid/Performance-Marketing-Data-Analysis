...
FROM marketing_and_product_performance;
 ...
 SELECT AVG(Conversition_rate) AS average_Conversition_rate 
 FROM marketing_and_product_performance;
...
SELECT 
    ...
    AVG(Conversition_rate) AS avg_Conversition_rate,
...
SELECT 
    CASE 
        WHEN Conversition_rate < 20 THEN 'Low Conversion'
        WHEN Conversition_rate BETWEEN 20 AND 50 THEN 'Medium Conversion'
        ELSE 'High Conversion'
    END AS Conversition_category,
    COUNT(*) AS num_campaigns,
    AVG(Profit_Margin) AS avg_Profi_Margin,
    SUM(Budget) AS total_budget,
    SUM(Revenue_generated) AS total_revenue
FROM marketing_and_product_performance
GROUP BY Conversition_category;
